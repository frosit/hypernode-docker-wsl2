#!/usr/bin/env bash
# DNS challenge hook for dehydrated
# Original: https://github.com/bennettp123/dehydrated-email-notify-hook/blob/master/hook.sh
# modified by: Fabio Ros
#
...
EOF
    ...
    echo -e "Add DNS record:"

    show_DNS_challenge "$MESSAGE"
...
function show_DNS_challenge(){
    local MESSAGE=$1
    mkdir -p /data/web/.tmp
    echo "$MESSAGE" >> /data/web/.tmp/dns.txt
    echo -e "$MESSAGE"
    send_push "$MESSAGE"
}
...
function send_push(){
    ...
    if [[ -z "${PUSHBULLET_ACCESS_TOKEN}" ]]; then
        echo -e "Set the env var: PUSHBULLET_ACCESS_TOKEN to enable pushes"
    else
        curl -s -u $PUSHBULLET_ACCESS_TOKEN: -X POST https://api.pushbullet.com/v2/pushes --header 'Content-Type: application/json' --data-binary '{"type": "note", "title": "'"DNS challenge"'", "body": "'"$MESSAGE"'"}' >/dev/null 2>&1
    fi
}
...
if [[ "${HANDLER}" =~ ^(deploy_challenge|clean_challenge|deploy_cert|unchanged_cert)$ ]]; then
  "$HANDLER" "$@"
    if [[ "$HANDLER" == 'unchanged_cert' ]] || [[ "$HANDLER" == "deploy_cert" ]]; then
        /usr/bin/hypernode-ssl-config-generator
        touch /data/web/nginx/letsencrypt.flag
        echo -e "check /etx/nginx/app for certs dir, if not there, run nginx_config_reloader"
    fi
fi